...
is_live_feed = False
is_recording = False
is_recording_enabled = False # New state for the recording toggle
...
def reset_camera():
    """Stops, re-configures, and restarts the camera to ensure a clean state."""
    global picam2
    print("🔄 RESETTING CAMERA to clean state...")
    try:
        picam2.stop()
        print("  - Camera stopped.")
        
        t = libcamera.Transform(hflip=False, vflip=False)
        config = picam2.create_video_configuration(main={"size": current_config["resolution"]}, transform=t)
        picam2.configure(config)
        print("  - Camera reconfigured.")
        
        picam2.start()
        print("  - Camera restarted.")
        print("✅ Camera reset successful.")
    except Exception as e:
        print(f"❌ CRITICAL: Failed to reset camera: {e}")
...
def stop_video_recording():
    ...
    try:
        ...
        # CRITICAL FIX 2.0: The camera object must be completely reset.
        reset_camera()
...
def controller_loop():
    ...
    while not stop_event.is_set():
        ...
        if current_mode == "detection":
            ...
            if motion_event.is_set():
                ...
                if not detection_active:
                    print(f"\n🚨 MOTION DETECTED - Starting detection session")
        ...
        # ========== MANUAL MODE ==========
            ...
            if is_live_feed:
                # Live feed active - stream and optionally record
...
def stream():
    def generate():
        ...
        print(is_live_feed, streaming_event.is_set(), is_recording)
...
def update_settings():
    global current_mode, is_live_feed, is_recording_enabled
    ...
    if 'recording_enabled' in data:
        new_value = bool(data['recording_enabled'])
        if new_value != is_recording_enabled:
            is_recording_enabled = new_value
            print(f"🔄 Recording enabled set to: {is_recording_enabled}")
            # If recording is disabled mid-session, stop it now
            if not is_recording_enabled and is_recording:
                stop_video_recording()
            broadcast_status()
        ...
        <div class="lg:col-span-8 space-y-6">
            <div class="bg-black rounded-2xl overflow-hidden aspect-video relative ring-1 ring-slate-700 shadow-2xl group">
                ...
                <div class="absolute top-4 left-4 flex gap-2 pointer-events-none">
                    <div id="liveBadge" class="hidden px-3 py-1 bg-blue-600 text-[10px] font-black rounded-full shadow-lg border border-blue-500">LIVE</div>
                    <div id="detectionBadge" class="hidden px-3 py-1 bg-orange-500 text-[10px] font-black rounded-full shadow-lg border border-orange-400">DETECTION</div>
                    <div id="recBadge" class="hidden px-3 py-1 bg-red-600 text-[10px] font-black rounded-full shadow-lg border border-red-500 status-pulse">REC</div>
                </div>
            ...
            <!-- Diagnostics -->
            <div class="bg-slate-800/50 backdrop-blur p-4 rounded-2xl border border-slate-700/50 flex flex-wrap gap-6 items-center justify-between">
                <div class="flex-1 min-w-[200px]">
                    <h2 class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Hardware Sensors</h2>
                    <div class="flex gap-6">
                        <div class="flex items-center gap-3">
                            <div id="pirDot" class="w-6 h-6 rounded-full bg-slate-700 transition-colors shadow-sm"></div>
                            <div>
                        ...
                        <div class="flex items-center gap-3">
                            <div id="micDot" class="w-6 h-6 rounded-full bg-slate-700 transition-colors shadow-sm"></div>
                            <div>
                ...
                <div id="manualControls" class="flex-1 min-w-[240px] flex gap-2">
                    <button id="liveFeedBtn" onclick="toggleLiveFeed()" 
                        class="flex-grow py-3 bg-blue-600 hover:bg-blue-500 rounded-xl font-black text-sm tracking-tight transition-all active:scale-95 shadow-lg shadow-blue-900/20">
                        START LIVE
                    </button>
                    <button id="recToggleBtn" onclick="toggleRecording()" class="w-20 py-3 bg-slate-700 rounded-xl font-bold text-sm">REC</button>
                </div>
                <div id="detectionControls" class="flex-1 hidden min-w-[240px]">
                    <div class="bg-green-500/10 text-green-500 border border-green-500/20 p-3 rounded-xl flex items-center justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <div class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                            <p class="text-xs font-bold">SYSTEM ARMED</p>
                        </div>
                        <button id="recToggleBtnDetection" onclick="toggleRecording()" class="px-4 py-1 bg-slate-700 rounded-lg font-bold text-xs">REC</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function updateUIState() {
        log(`UI State: Mode=${state.mode}, LiveReq=${state.liveRequest}, Motion=${state.serverMotionActive}, Rec=${state.isRecording}, RecEnabled=${state.isRecordingEnabled}`);
        ...
        // 3. Badges
        liveBadge.classList.toggle('hidden', !state.liveRequest);
        detectionBadge.classList.toggle('hidden', !state.serverMotionActive || state.mode !== 'detection');
        recBadge.classList.toggle('hidden', !state.isRecording);
        ...
        // 5. REC Toggle Buttons
        const recToggles = [document.getElementById('recToggleBtn'), document.getElementById('recToggleBtnDetection')];
    ...
    socket.on('status_update', (data) => {
        log(`Status: mode=${data.mode}, live=${data.live_feed}, motion=${data.motion}, rec=${data.recording}, rec_enabled=${data.recording_enabled}`);
        state.mode = data.mode;
        state.liveRequest = data.live_feed;
        state.serverMotionActive = data.motion;
        state.isRecording = data.recording;
        state.isRecordingEnabled = data.recording_enabled;
        updateUIState();
    });
    ...
    socket.on('sensor_debug', (data) => {
        ...
        document.getElementById('pirDot').className = data.pir ? 'w-6 h-6 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'w-6 h-6 rounded-full bg-slate-700';
        document.getElementById('micDot').className = data.mic ? 'w-6 h-6 rounded-full bg-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.5)]' : 'w-6 h-6 rounded-full bg-slate-700';
    });
    ...
    function toggleRecording() {
        const newState = !state.isRecordingEnabled;
        log(`Toggle recording enabled: ${newState}`);
        fetch('/api/settings', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({recording_enabled: newState})
        });
        state.isRecordingEnabled = newState;
        updateUIState();
    }
